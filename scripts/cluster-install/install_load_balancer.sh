...
then
  # 2 managers
  cat $5/certs/load_balancer_cert.pem $5/certs/load_balancer_key.pem > $5/certs/cert.pem
  sudo mv $5/certs/cert.pem /etc/haproxy
  sudo chown haproxy. /etc/haproxy/cert.pem
  sudo chmod 400 /etc/haproxy/cert.pem

  sudo cp $5/certs/ca.crt /etc/haproxy
  ...
  listen manager
      ...
      server manager_$3 $4 maxconn 100 ssl check check-ssl port 443 ca-file /etc/haproxy/ca.crt" | sudo tee /etc/haproxy/haproxy.cfg
else
  # 3 managers
